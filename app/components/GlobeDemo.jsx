...
export function GlobeDemo({ reducedMotion = false }) {
  const sampleArcs = useMemo(() => {
    return ARC_BLUEPRINTS.map((arc, index) => ({
      ...arc,
      color: COLORS[(arc.order + index) % COLORS.length],
    }));
  }, []);
  ...
  const globeConfig = useMemo(
    () => ({
      ...
      autoRotateSpeed: reducedMotion ? 0 : BASE_GLOBE_CONFIG.autoRotateSpeed,
    }),
    [reducedMotion],
  );

  return (
    <div className="relative flex h-full w-full items-center justify-center overflow-hidden rounded-3xl bg-black">
      <motion.div
      ...
      <div className="absolute inset-0 z-20 pointer-events-none bg-gradient-to-t from-black/35 via-transparent to-black/5" />
    </div>
  );
}
